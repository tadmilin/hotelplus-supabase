...
## 🔥 UPDATE: แก้ปุ่มลบ Drive ให้ทำงานถูกต้อง

### **ปัญหาเดิม:**
- ❌ ปุ่มลบ Drive ลบจาก `google_drives` (global) - กระทบทุกคน
- ❌ ระบบ sync ล้าง drives ทั้งหมดแล้วเพิ่มใหม่ - ลบกลับมาเอง
- ❌ แต่ละ user ไม่มีการแยกการแสดงผล drives

### **การแก้ไข:**
1. ✅ **ปุ่มลบ Drive ใช้ได้แล้ว** - ลบจาก `user_drive_access` แทน (เฉพาะ user นี้)
2. ✅ **ไม่กระทบ user อื่น** - Google drives ยังอยู่ในระบบ
3. ✅ **Sync ไม่ล้าง drives** - ใช้ UPSERT แทน DELETE + INSERT
4. ✅ **แต่ละ user มี drives แยกกัน** - ดึงจาก user_drive_access
5. ✅ **ปุ่ม Sync ชัดเจน** - UI แยกปุ่ม "เลือก Drives" และ "Sync Drives"

### **ไฟล์ที่แก้ไข:**
- `app/api/drive/user-drives/route.ts` - DELETE ลบจาก user_drive_access
- `app/api/drive/sync/route.ts` - เปลี่ยนจาก DELETE+INSERT → UPSERT
- `app/gpt-image/page.tsx` - อัปเดต confirm message และ comment
- `app/custom-prompt/page.tsx` - อัปเดต confirm message

### **วิธีใช้งาน:**
1. **Sync Drives**: คลิก "🔄 อัพเดทรายการ" - ดึง drives ใหม่จาก Google
2. **เลือก Drives**: คลิก "⚙️ เลือก Drives" - เลือกที่จะแสดง
3. **ลบ Drive**: คลิก 🗑️ ข้าง Drive name - ซ่อนออกจากรายการของคุณ
4. **เพิ่มกลับมา**: Sync ใหม่ → เลือก Drives อีกครั้ง
